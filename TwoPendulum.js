window.addEventListener("load", model, true);
function model() {
...
//	var k0 = 2 * Pi / T0;           	// масштаб частоты
   ...
   // var v = 0;				// скорость тела
    ...
    //var rw = canvas.width / 100;
	//var rh = canvas.height / 4;
...
//	var bound_x = x0;					// координаты закрепления первого маятника по Х
    ...
    slider_l.value = (l / l0).toFixed(1);
	number_l.value = (l / l0).toFixed(1);
	slider_fiZero.value = (fiZero / fiZero0).toFixed(1);
	number_fiZero.value = (fiZero / fiZero0).toFixed(1);
	...
	function setFiZero(new_fiZero)
	{
		fiZero = new_fiZero * fiZero0;
	}
    ...
    slider_l.oninput = function() {
		number_l.value = slider_l.value;
		setL(slider_l.value);
	};
    number_l.oninput = function() {
		slider_l.value = number_l.value;
		setL(number_l.value);
	};
	
	slider_fiZero.oninput = function() {
		number_fiZero.value = slider_fiZero.value;
		setFiZero(slider_fiZero.value);
	};
    number_fiZero.oninput = function() {
		slider_fiZero.value = number_fiZero.value;
		setFiZero(number_fiZero.value);
	};
	...
	/* 	draw_spring(
		...
		ctx.fill();
		ctx.lineWidth = 7;
		ctx.strokeStyle = '#093300';
		ctx.stroke();
		
		// Рисование закрепления первого маятника
		ctx.lineWidth = 6;
        ctx.strokeStyle = "#cbca73";
		ctx.beginPath();
